...
%%% Another half-cycle interval adjacent to the one in which the peak value is located %%     
 ...
 Vp=zeros(length(Signal_Filter),1); 
 if max(abs(half_cycle_interval_1))>=max(abs(half_cycle_interval_2))
  Vp(1:zero_point_1-1,1)=0;  
  Vp(zero_point_1:zero_point_4)=Signal_Filter(zero_point_1:zero_point_4); 
  Vp(zero_point_4+1:length(Signal_Filter),1)=0;
  Tp=zero_point_4*dt-zero_point_1*dt; 
 else
  Vp(1:zero_point_3-1,1)=0;  
  Vp(zero_point_3:zero_point_2)=Signal_Filter(zero_point_3:zero_point_2); 
  Vp(zero_point_2+1:length(Signal_Filter),1)=0;
  Tp=zero_point_2*dt-zero_point_3*dt; 
 end   

%%%code to indicate the number of peak-points in the pulse signal %%%
 [~,in1]=findpeaks(abs(Vp));
 if length(in1)==2
  code=1;
 else 
  code=0;
